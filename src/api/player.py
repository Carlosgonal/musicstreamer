...
def source():
    ...
    if source == "radio":
        ...
        set_state(
            ...
            artwork_url=current_station.get("image_url"),
        )
        return jsonify({
            "player": get_player_status(),
            "radio": radio_status,
...
@player_api.post("/toggle")
def toggle():
    payload = request.get_json(silent=True) or {}
    player = get_player_status()
    source = str(payload.get("source") or player.get("source") or "spotify").strip().lower()
    station_id = str(payload.get("station_id", "")).strip() or None
    ...
    if source == "spotify":
        spotify_status = get_spotify_status()
        spotify_player = spotify_status.get("player") or {}
        action = "pause" if spotify_player.get("is_playing") else "play"

        try:
            spotify_status = control_playback(action)
        except RuntimeError as error:
            return jsonify({"error": str(error)}), 503
